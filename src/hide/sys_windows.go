...
/*
sys CreateFile( h syscall.Handle,dwDesiredAccess uint32,dwShareMode uint32, lpSecurityAttributes uintptr,dwCreationDisposition uint32,dwFlagsAndAttributes uint32,hTemplateFile syscall.Handle)(hd syscall.Handle,err error)=kernel32.CreateFile
sys ReadFile(h syscall.Handle,lpBuffer *byte,nNumberOfBytesToRead uint32, lpNumberOfBytesRead *uint32 ,LPOVERLAPPED *syscall.Overlapped)(status bool,err error)=kernel32.ReadFile
sys WriteFile(h syscall.Handle,lpBuffer *byte,nNumberOfBytesToWrite uint32, lpNumberOfBytesWritten *uint32 ,LPOVERLAPPED *syscall.SecurityAttributes)(status bool,err error)=kernel32.WriteFile
sys CloseHandle(h syscall.Handle)(err error)=kernel32.CloseHandle
*/
//go:generate go run $GOROOT/src/syscall/mksyscall_windows.go -systemdll -output zsys_windows.go sys_windows.go
